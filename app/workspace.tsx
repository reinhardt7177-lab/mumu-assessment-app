...
export default function Workspace({ initialAssessments }: { initialAssessments: AssessmentRecord[] }) {
  ...
  return <div className="assessment-home">
    <section className="home-hero"><div><p className="kicker">이준용 선생님의 평가 워크스페이스</p><h1>학생의 답안에서<br />다음 배움을 발견하세요.</h1><p>초등 성취기준으로 설계하고, 실제 제출된 답안을 근거로 평가합니다.</p><Link className="curriculum-bridge-link" href="/curriculum">교육과정 성장 대시보드에서 학기 흐름 보기 →</Link></div><button type="button" className="create-button" onClick={() => setCreating(true)}>＋ 새 평가 만들기</button></section>
