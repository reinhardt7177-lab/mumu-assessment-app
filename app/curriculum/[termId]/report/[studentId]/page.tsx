...
function describeProgress(first: Level | undefined, current: Level | undefined) {
  if (!first || !current) return "누적된 최종 판단 근거가 아직 없습니다.";
  if (current === "판단 보류") return `최초 ${first} · 현재 추가 근거 필요`;
  if (first === "판단 보류") return `최초 추가 근거 필요 → 현재 ${current} · 성취 근거 확인`;
  const comparison = levelRank(current) - levelRank(first);
  if (comparison > 0) return `최초 ${first} → 현재 ${current} · 향상 확인`;
  if (comparison === 0) return `최초 ${first} → 현재 ${current} · 일관된 수행 확인`;
  return `최초 ${first} → 현재 ${current} · 수행 맥락에 따른 차이, 추가 근거 확인`;
}
...
export default async function StudentSemesterReport({ params }: { params: Promise<{ termId: string; studentId: string }> }) {
  ...
  const rows = standards.map(standard => {
    ...
    return { ...standard, judgement, history, first, current, progressDescription: describeProgress(first, current) };
  });
  const completedGrowth = feedback.filter(item => item.status === "completed" && item.reassessments.some(link => link.independent));
  const completedGrowthCount = new Set(completedGrowth.map(item => item.standardCode)).size;
