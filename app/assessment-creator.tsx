...
export default function AssessmentCreator({ onClose, onCreated }: { onClose: () => void; onCreated: (a: AssessmentRecord) => void }) {
  ...
  const generate = async () => {
    setBusy(true); setError(""); setGenerated(false);
    try {
      const data = await requestJson<{ questions: Omit<AssessmentQuestion, "id">[] }>("/api/ai/questions", { method: "POST", body: JSON.stringify({ title, subject, learningGoal: goal, standards, count: 3 }), signal: AbortSignal.timeout(45000) });
  ...
  return <dialog ref={dialog} className="create-modal real-dialog" aria-labelledby="creator-title" onCancel={event => { if (busy) event.preventDefault(); else onClose(); }}>
    ...
    {step === 2 && <div className="wizard-body question-builder"><div className="question-builder-heading"><p className="wizard-guide">성취기준 → 문항 → 판단 근거를 연결합니다. 총 {questions.length}문항 · {total}점</p><button className="ai-question-button" disabled={busy || questions.length > 0} onClick={generate}>{busy ? "Luna가 생성하는 중…" : "✦ Luna 문항 초안"}</button></div>{questions.length > 0 && <small>기존 문항을 보호하기 위해 AI 생성은 빈 문항 목록에서만 실행합니다.</small>}{generated && <p className="save-notice">AI 초안입니다. 내용·정답 가능성·발달 수준을 교사가 검토해 주세요.</p>}<div className="question-list">{questions.map((q, index) => <article className="question-editor" key={q.id}><div className="question-editor-head"><strong>문항 {index + 1}</strong><button onClick={() => setQuestions(current => current.filter(item => item.id !== q.id))}>삭제</button></div><label>문항 내용<textarea value={q.prompt} maxLength={2000} onChange={e => updateQuestion(q.id, { prompt: e.target.value })} /></label><div className="question-meta"><label>성취기준<select value={q.standardCode} onChange={e => updateQuestion(q.id, { standardCode: e.target.value })}>{standards.map(s => <option key={s.code}>{s.code}</option>)}</select></label><label>평가 기준<select value={q.criterion} onChange={e => updateQuestion(q.id, { criterion: e.target.value })}>{rubric.map(r => <option key={r.name}>{r.name}</option>)}</select></label><label>배점<input type="number" min={1} max={100} value={q.points} onChange={e => updateQuestion(q.id, { points: Number(e.target.value) })} /></label></div></article>)}</div><button className="add-question-button" disabled={questions.length >= 20 || busy} onClick={addQuestion}>＋ 문항 직접 추가</button></div>}
