          ...
          <section className="create-modal" aria-labelledby="create-title">
            ...
            {step === 3 && (
              <div className="wizard-body question-builder">
                <div className="question-builder-heading"><div><p className="wizard-guide">선택한 성취기준을 실제로 확인할 문항을 만드세요. 각 문항은 성취기준과 평가 기준에 연결됩니다.</p><span>{selectedStandards.map((standard) => standard.code).join(" · ")} · 총 {questions.length}문항 · {questionPoints}점</span></div><button type="button" className="ai-question-button" onClick={generateQuestionDrafts} disabled={questionsGenerating}>{questionsGenerating ? "Luna가 문항을 만드는 중…" : "✦ Luna 문항 초안 생성"}</button></div>
                {questionsGenerated && <StatusToast>Luna가 선택한 성취기준과 학습 목표를 바탕으로 문항 초안을 만들었습니다. 교사가 반드시 검토해 주세요.</StatusToast>}
