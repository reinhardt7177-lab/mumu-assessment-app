...
초등만 다룬다. 교육과정·차시 수업안에서 성취기준을 해석한 뒤 루브릭과 편집 가능한 평가지 초안을 만드는 과정이 응답 방법보다 먼저다. 학생에게 QR로 열리는 화면은 시험지여야 한다. AI 모델명은 화면에 노출하지 않고 교사에게는 "문항 초안"으로 안내하며, 실제 모델은 비용·접근 가능 여부를 검증해 선택한다. 교사가 결과를 최종 판단하며 상중하 환산 기준은 교사 설정임을 명시한다. 학생 성장 요약은 S-W-A-T(강점·보완점·실행학습·다음 목표)로 정리한다. 운영 저장과 데모는 섞지 않는다.
