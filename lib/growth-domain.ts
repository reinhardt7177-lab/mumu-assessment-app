...
export const evidenceInputSchema = z.object({
  ...
  modality: z.enum(["text", "photo", "speech", "observation", "chat"]),
  sourceKind: z.enum(["student_response", "handwritten_work", "recording", "teacher_observation", "chatbot_transcript"]),
  assistanceLevel: z.enum(["independent", "teacher_prompt", "step_hint", "example", "scaffolded"]),
  originalText: z.string().trim().min(1).max(50000).optional(),
