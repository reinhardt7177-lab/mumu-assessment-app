...
function signatureMatches(bytes: Uint8Array, mime: string) {
  ...
  if (mime === "audio/mp4") return ascii(bytes, 4, 4) === "ftyp";
...
export async function validateEvidenceFile(file: File, modality: "photo" | "speech") {
  if (!process.env.BLOB_READ_WRITE_TOKEN) throw new AppError(503, "비공개 사진·녹음 저장소 연결 준비 중입니다.");
  const mimeType = normalizedMime(file);
  if (!allowed[modality].has(mimeType as never)) throw new AppError(415, modality === "photo" ? "JPG·PNG·WebP 사진만 올릴 수 있습니다." : "WebM·M4A·MP3·WAV 녹음만 올릴 수 있습니다.");
...
export async function storePrivateEvidence(input: { attemptId: string; questionId: string; modality: "photo" | "speech"; bytes: Uint8Array; mimeType: string }) {
